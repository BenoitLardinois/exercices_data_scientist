...
def shortest(handle)
  ...
  puts handle.min { |a, b| a.length <=> b.length }
  
end

def symbol_letter(handle)
  puts "-- 3 Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère) --"
  puts handle.count { |a| a.length == 6 }
end

def capital_letter(handle)
  puts "-- 4 Combien commencent par une majuscule (première lettre juste après le @) ? --"
  capital_letters = ('A'..'Z')
  puts handle.count {|x| capital_letters.include?(x[1])}
end

def alphabetical_order(handle)
  #prendre en compte les _ ???
  puts "-- 5 Trier la liste par ordre alphabétique --"
  puts handle.sort_by(&:downcase)
end

def smallest_biggest(handle)
  puts "-- 6 Trier la liste par taille (du plus petit au plus grand) --"
  puts handle.sort_by(&:length)
end

def position(handle)
  puts "-- 7 Quelle est la position dans l'array de la personne @epenser ? --"
  puts handle.find_index { |item| item.downcase == "@epenser" }
end

def distribution(handle)
  puts "-- 8 Répartition des handles par taille --"
  min = handle.min_by { |q| q.size }
  min = min.length
  max = handle.max_by { |q| q.size }
  max = max.length
  i = min 
  while i <= max
    n = handle.count { |content| content.length == i }
    if n != 0
      puts " #{n} handle de #{i - 1} caractère(s)"
    end
    i += 1
  end
end
...
def perform
  ...
  how_much(handle)
  shortest(handle)
  symbol_letter(handle)
  capital_letter(handle)
  alphabetical_order(handle)
  smallest_biggest(handle)
  position(handle)
  distribution(handle)
end
